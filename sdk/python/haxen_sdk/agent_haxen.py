...
class AgentHaxen:
    """
    Haxen Server Communication handler for Agent class.

    This class encapsulates all Haxen server communication functionality including:
    - Agent registration with Haxen server
    ...
    def __init__(self, agent_instance):
        """
        Initialize the Haxen handler with a reference to the agent instance.
    ...
    async def register_with_haxen_server(self, port: int):
        """Register this agent node with Haxen server"""
        # Import the callback URL resolution function
        from haxen_sdk.agent import (
        ...
        try:
            log_debug(
                f"Attempting to register with Haxen server at {self.agent.haxen_server}"
            )
            ...
            if success:
                ...
                log_success(f"Registered node '{self.agent.node_id}' with Haxen server")
                self.agent.haxen_connected = True

                # Attempt DID registration after successful Haxen registration
                ...
                self.agent.haxen_connected = False

        except Exception as e:
            self.agent.haxen_connected = False
            if self.agent.dev_mode:
                log_warn(f"Haxen server not available: {e}")
                log_setup("Running in development mode - agent will work standalone")
                log_info(
                    f"To connect to Haxen server, start it at {self.agent.haxen_server}"
                )
            else:
                log_error(f"Failed to register with Haxen server: {e}")
                if (
    ...
    def send_heartbeat(self):
        """Send heartbeat to Haxen server"""
        if not self.agent.haxen_connected:
            return  # Skip heartbeat if not connected to Haxen

        try:
            response = requests.post(
                f"{self.agent.haxen_server}/api/v1/nodes/{self.agent.node_id}/heartbeat",
    ...
    def heartbeat_worker(
        ...
        if not self.agent.haxen_connected:
            log_heartbeat("Heartbeat worker skipped - not connected to Haxen server")
            return
    ...
    async def notify_shutdown(self) -> bool:
        """
        Notify Haxen server of graceful shutdown.
    ...
    def setup_fast_lifecycle_signal_handlers(
        ...
        - Only intercepts SIGTERM to mark the agent offline and notify Haxen immediately.
        - Leaves SIGINT (Ctrl+C) to uvicorn so its shutdown hooks run and resources are cleaned up.
        """

        def signal_handler(signum: int, frame) -> None:
            """Handle SIGTERM: mark offline, notify Haxen, then re-emit the signal for default handling."""
            ...
            # Best-effort immediate notification to Haxen
            try:
    ...
    async def register_with_fast_lifecycle(
        ...
        """
        from haxen_sdk.agent import _build_callback_candidates, _resolve_callback_url
        ...
        try:
            if self.agent.dev_mode:
                log_info(
                    f"Fast registration with Haxen server at {self.agent.haxen_server}"
                )
