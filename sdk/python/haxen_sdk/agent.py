...
from haxen_sdk.agent_ai import AgentAI
from haxen_sdk.agent_haxen import AgentHaxen
from haxen_sdk.agent_mcp import AgentMCP
from haxen_sdk.agent_registry import clear_current_agent, set_current_agent
from haxen_sdk.agent_server import AgentServer
...
from haxen_sdk.execution_context import ExecutionContext, get_current_context
from haxen_sdk.did_manager import DIDManager
from haxen_sdk.vc_generator import VCGenerator
from haxen_sdk.mcp_client import MCPClientRegistry
from haxen_sdk.mcp_manager import MCPManager
from haxen_sdk.memory import MemoryClient, MemoryInterface
from haxen_sdk.memory_events import MemoryEventClient
from haxen_sdk.logger import log_debug, log_error, log_info, log_warn
from haxen_sdk.router import AgentRouter
from haxen_sdk.connection_manager import ConnectionManager
from haxen_sdk.types import AgentStatus, AIConfig, MemoryConfig
from haxen_sdk.multimodal_response import MultimodalResponse
from haxen_sdk.async_config import AsyncConfig
from haxen_sdk.async_execution_manager import AsyncExecutionManager
from haxen_sdk.pydantic_utils import convert_function_args, should_convert_args
...
class Agent(FastAPI):
    """
    Haxen Agent - FastAPI subclass for creating AI agent nodes.

    The Agent class is the core component of the Haxen SDK that enables developers to create
    intelligent agent nodes. It inherits from FastAPI to provide HTTP endpoints and integrates
    with the Haxen ecosystem for distributed AI workflows.

    Key Features:
    - Decorator-based reasoner and skill registration
    - Cross-agent communication via the Haxen execution gateway
    ...
    def __init__(
        self,
        node_id: str,
        haxen_server: str = "http://localhost:8080",
        version: str = "1.0.0",
        ...
        **kwargs,
        ...
        (deterministic functions) while integrating with the Haxen ecosystem for distributed
        ...
        Args:
            node_id (str): Unique identifier for this agent node. Used for routing and
                          ...
                          within your Haxen ecosystem.
            haxen_server (str, optional): URL of the Haxen server for registration and
            ...
            callback_url (str, optional): Explicit callback URL for Haxen server to reach this agent.
        ...
        Example:
            ...
            app = Agent(
                ...
                haxen_server="https://haxen.company.com",
                version="2.1.0",
                ai_config=AIConfig(
                    provider="openai",
                    model="gpt-4",
        ...
        # Initialize HaxenClient with async configuration
        self.client = HaxenClient(base_url=haxen_server, async_config=self.async_config)
        ...
        # Add connection management for resilient Haxen server connectivity
        self.connection_manager: Optional[ConnectionManager] = None

        # Initialize handlers
        self.ai_handler = AgentAI(self)
        self.mcp_handler = AgentMCP(self)
        self.haxen_handler = AgentHaxen(self)
        ...
        # Setup standard Haxen routes and memory event listeners
        self.server_handler.setup_haxen_routes()
        ...
        max_calls_env = os.getenv("HAXEN_AGENT_MAX_CONCURRENT_CALLS")
        if max_calls_env:
            try:
                ...
                log_warn(
                    f"Invalid HAXEN_AGENT_MAX_CONCURRENT_CALLS='{max_calls_env}', defaulting to {default_limit}"
                )
        else:
            self._max_concurrent_calls = default_limit
    ...
    def handle_serverless(self, event: dict) -> dict:
        ...
        Special Endpoints:
            - /discover: Returns agent metadata for Haxen server registration
        ...
        # Auto-register with Haxen if needed (for execution requests)
        if self.auto_register and not self.haxen_connected:
            try:
                # Attempt registration (non-blocking)
                self.haxen_handler._register_agent()
                self.haxen_connected = True
            except Exception as e:
                if self.dev_mode:
    ...
    def _handle_discovery(self) -> dict:
        ...
        for automatic registration with the Haxen server.
    ...
    def _initialize_did_system(self):
        ...
        try:
            ...
            self.did_manager = DIDManager(self.haxen_server, self.node_id)

            # Initialize VC Generator
            self.vc_generator = VCGenerator(self.haxen_server)
    ...
    def _register_memory_event_listeners(self):
        ...
        if not self.memory_event_client:
            self.memory_event_client = MemoryEventClient(
                self.haxen_server, self._get_current_execution_context()
            )
    ...
    def _apply_discovery_response(self, payload: Optional[Dict[str, Any]]) -> None:
        """Update agent networking state from Haxen discovery response."""
        ...
        if resolved and resolved != self.base_url:
            log_debug(f"Applying resolved callback URL from Haxen: {resolved}")
    ...
    def _setup_haxen_routes(self):
        """Delegate to server handler for route setup"""
        return self.server_handler.setup_haxen_routes()
    ...
    def reasoner(self, path: Optional[str] = None, name: Optional[str] = None):
        ...
        It automatically handles input/output schema generation and integrates with the Haxen's AI capabilities.

        Args:
            path (str, optional): The API endpoint path for this reasoner. Defaults to /reasoners/{function_name}.
            name (str, optional): Explicit Haxen registration ID. Defaults to the function name.
        """
    ...
    def skill(
        ...
        - Enables cross-agent communication via the Haxen execution gateway
        ...
        Returns:
            Callable: The decorated function with enhanced Haxen integration.
    ...
    async def call(self, target: str, *args, **kwargs) -> dict:
        """
        Initiates a cross-agent call to another reasoner or skill via the Haxen execution gateway.

        This method allows agents to seamlessly communicate and utilize reasoners/skills
        deployed on other agent nodes within the Haxen ecosystem. It properly propagates
        ...
        **Return Type**: Always returns JSON/dict objects, similar to calling any REST API.
        ...
        if self.dev_mode:
            from haxen_sdk.execution_context import get_current_context
            from haxen_sdk.logger import log_debug
        ...
        # DISABLED: Same-agent call detection - Force all calls through Haxen server
        # This ensures all app.call() requests go through the Haxen server for proper
        # workflow tracking, execution context, and distributed processing
        from haxen_sdk.logger import log_debug

        log_debug(f"Cross-agent call to: {target}")

        # Check if Haxen server is available for cross-agent calls
        if not self.haxen_connected:
            from haxen_sdk.logger import log_warn

            log_warn(
                f"Haxen server unavailable - cannot make cross-agent call to {target}"
            )
            raise Exception(
                f"Cross-agent call to {target} failed: Haxen server unavailable. Agent is running in local mode."
            )

        # Use the enhanced HaxenClient to make the call via execution gateway
        try:
            async with self._limit_outbound_calls():
                ...
                use_async_execution = (
                    self.async_config.enable_async_execution and self.haxen_connected
                )
    ...
    async def _get_async_execution_manager(self) -> AsyncExecutionManager:
        ...
        if self._async_execution_manager is None:
            ...
            self._async_execution_manager = AsyncExecutionManager(
                base_url=self.haxen_server, config=self.async_config
            )
            # Start the manager
    ...
    async def _cleanup_async_resources(self) -> None:
        ...
        if getattr(self, "client", None) is not None:
            try:
                ...
                if self.dev_mode:
                    log_debug("HaxenClient resources closed")
            except Exception as e:
                if self.dev_mode:
                    log_debug(f"Error closing HaxenClient resources: {e}")

    def note(self, message: str, tags: List[str] = None) -> None:
        """
        Add a note to the current execution for debugging and tracking purposes.

        This method sends a note to the Haxen server asynchronously without blocking
        the current execution. The note is automatically associated with the current
        execution context and can be viewed in the Haxen UI for debugging and monitoring.
    ...
    def serve(  # pragma: no cover - requires full server runtime integration
        ...
        **kwargs,
        ...
        """
        Start the agent node server with intelligent port management and Haxen integration.

        This method launches the agent as a FastAPI server that can receive reasoner and skill
        requests from other agents via the Haxen execution gateway. It handles automatic
        registration with the Haxen server, heartbeat management, and graceful shutdown.

        The server provides:
        - RESTful endpoints for all registered reasoners and skills
        - Health check endpoints for monitoring
        - MCP server status and management endpoints
        - Automatic Haxen server registration and heartbeat
        ...
        Args:
            ...
            heartbeat_interval (int): The interval in seconds for sending heartbeats to the Haxen server.
        ...
        Integration with Haxen:
            - Automatically registers with Haxen server on startup
            - Sends periodic heartbeats to maintain connection
            - Receives execution requests via Haxen's routing system
