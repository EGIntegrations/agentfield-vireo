...
class HaxenClient:
    ...
    def _sync_request(method: str, url: str, **kwargs):
        ...
        if "User-Agent" not in kwargs["headers"]:
            kwargs["headers"]["User-Agent"] = "HaxenSDK/1.0"
    ...
    def register_node(self, node_data: Dict[str, Any]) -> Dict[str, Any]:
        """Register agent node with Haxen server"""
    ...
    async def register_agent(
        ...
        """Register or update agent information with Haxen server."""
        try:
            registration_data = {
                "id": node_id,
                "team_id": "default",
    ...
    async def send_enhanced_heartbeat(
        ...
        """
        Send enhanced heartbeat with status and MCP information to Haxen server.
    ...
    async def notify_graceful_shutdown(self, node_id: str) -> bool:
        """
        Notify Haxen server that the agent is shutting down gracefully.
    ...
    async def close_async_execution_manager(self) -> None:
        ...
        This should be called when the HaxenClient is no longer needed
