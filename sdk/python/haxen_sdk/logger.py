"""
Haxen SDK Logging Utility

This module provides a centralized logging system for the Haxen SDK that:
...
class HaxenLogger:
    """
    Centralized logger for Haxen SDK with configurable verbosity and payload truncation.

    Supports runtime log level changes (e.g., for dev_mode).
    """

    def __init__(self, name: str = "haxen_sdk"):
        self.logger = logging.getLogger(name)
        self._setup_logger()

        # Configuration from environment variables - default to WARNING (only important events)
        self.log_level = os.getenv("HAXEN_LOG_LEVEL", "WARNING").upper()
        self.truncate_length = int(os.getenv("HAXEN_LOG_TRUNCATE", "200"))
        self.show_payloads = os.getenv("HAXEN_LOG_PAYLOADS", "false").lower() == "true"
        self.show_tracking = os.getenv("HAXEN_LOG_TRACKING", "false").lower() == "true"
        self.show_fire = os.getenv("HAXEN_LOG_FIRE", "false").lower() == "true"
    ...
    def _format_payload(self, payload: Any) -> str:
        ...
        if not self.show_payloads:
            return "[payload hidden - set HAXEN_LOG_PAYLOADS=true to show]"
    ...
    def track(self, message: str, **kwargs):
        """Log tracking messages (controlled by HAXEN_LOG_TRACKING)"""
        if self.show_tracking:
            self.logger.debug(f"🔍 TRACK: {self._truncate_message(message)}")

    def fire(self, message: str, payload: Optional[Any] = None, **kwargs):
        """Log fire-and-forget workflow messages (controlled by HAXEN_LOG_FIRE)"""
        if self.show_fire:
...
def get_logger(name: str = "haxen_sdk") -> HaxenLogger:
    """Get or create a Haxen SDK logger instance"""
    global _global_logger
    if _global_logger is None:
        _global_logger = HaxenLogger(name)
