...
from haxen_sdk.logger import log_debug, log_error, log_info, log_success, log_warn
...
class AgentServer:
    """Server management functionality for Haxen Agent"""
    ...
    def setup_haxen_routes(self):
        """Setup standard routes that Haxen server expects"""
        ...
        async def shutdown_agent(request: Request):
            ...
            This endpoint allows the Haxen server to request a graceful shutdown
            ...
            try:
                ...
                # Set shutdown status
                from haxen_sdk.agent import AgentStatus

                self.agent._shutdown_requested = True
                self.agent._current_status = AgentStatus.OFFLINE

                # Notify Haxen server of shutdown initiation
                try:
        ...
        async def mcp_health():
            """Get MCP health information in the format expected by Haxen server"""
            if not self.agent.mcp_manager:
    ...
    async def _graceful_shutdown(self, timeout_seconds: int = 30):
        ...
        try:
            ...
            # Stop heartbeat
            try:
                if hasattr(self.agent, "haxen_handler") and self.agent.haxen_handler:
                    self.agent.haxen_handler.stop_heartbeat()
                    if self.agent.dev_mode:
            ...
            try:
                from haxen_sdk.agent_registry import clear_current_agent
    ...
    def serve(
        ...
        **kwargs,
        ...
        This method implements smart port resolution that seamlessly works with Haxen CLI
        or standalone execution. The port selection priority is:
        1. Explicit port parameter (highest priority)
        2. PORT environment variable (Haxen CLI integration)
        ...
        if port is None:
            ...
            if env_port and env_port.isdigit():
                ...
                if AgentUtils.is_port_available(suggested_port):
                    port = suggested_port
                    if self.agent.dev_mode:
                        log_debug(f"Using port from Haxen CLI: {port}")
                else:
                    # Haxen CLI suggested port is taken, find next available
                    try:
                        port = get_free_port(start_port=suggested_port)
                        if self.agent.dev_mode:
                            log_debug(
                                f"Haxen CLI port {suggested_port} taken, using {port}"
                            )
                    except RuntimeError:
                        port = get_free_port()  # Fallback to default range
                        if self.agent.dev_mode:
                            log_debug(f"Using fallback port: {port}")
            elif auto_port or os.getenv("HAXEN_AUTO_PORT") == "true":
        ...
        self.agent.haxen_handler.start_heartbeat(heartbeat_interval)
        ...
        self.agent.haxen_handler.setup_fast_lifecycle_signal_handlers()

        # Add startup event handler for resilient lifecycle
        @self.agent.on_event("startup")
        async def startup_resilient_lifecycle():
            """Resilient lifecycle startup: connection manager handles Haxen server connectivity"""
            ...
            config = ConnectionConfig(
                retry_interval=10.0,  # Check every 10 seconds for Haxen server
            ...
            def on_connected():
                if self.agent.dev_mode:
                    log_info("Connected to Haxen server - full functionality available")
                # Kick a heartbeat immediately so the control plane renews the lease
                try:
                    asyncio.create_task(
                        self.agent.haxen_handler.send_enhanced_heartbeat()
                    )
                except RuntimeError:
                ...
                if (
                    ...
                    self.agent._heartbeat_task = asyncio.create_task(
                        self.agent.haxen_handler.enhanced_heartbeat_loop(
                            heartbeat_interval
                        )
                    )
            ...
            def on_disconnected():
                if self.agent.dev_mode:
                    log_warn("Haxen server disconnected - running in local mode")
            ...
            # These work independently of Haxen server connection
            ...
            if connected:
                if self.agent.dev_mode:
                    log_info("Agent started with Haxen server connection")
            else:
                if self.agent.dev_mode:
                    log_info(
                        "Agent started in local mode - will connect to Haxen server when available"
                    )
        ...
        async def shutdown_cleanup():
            ...
            if getattr(self.agent, "client", None):
                try:
                    ...
                    if self.agent.dev_mode:
                        log_error(f"Haxen client shutdown error: {e}")
